...
| Core `ActivePrintContext` / `active_print_context` | Core printer | `printer_id -> core.printers.tenant_id` | No | Derived association | `EXISTS` through Core printer | Add the currently missing FK to Core printer; retain unique printer constraint |
