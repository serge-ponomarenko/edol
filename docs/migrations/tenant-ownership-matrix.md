...
| Hub `PrintJob` / `print_jobs` | Printer | `printer_id -> printers.tenant_id` | No | Derived association | `EXISTS` through printer | Hub V5 validates UUID ownership, makes the relationship mandatory, and removes the legacy integer column |
| Hub `PrinterStats` / `printer_stats` | Printer | `printer_id -> printers.tenant_id` | No | Derived association | `EXISTS` through printer | Hub V5 makes the printer FK non-null and retains one-stats-row-per-printer uniqueness |
| Hub `MaintenanceDefinition` / `maintenance_definition` | Printer | `printer_id -> printers.tenant_id` | No | Derived association | `EXISTS` through printer | Hub V5 makes the printer FK non-null after guarded validation |
